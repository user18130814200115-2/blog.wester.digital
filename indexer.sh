...
generate_html() {
         ...
         printf "All content on this blog also avaiable in the form of an RSS\nfeed at <a href=\"feed.xml\">feed.xml</a> and on gopher at <a href=\"tilde.club:70/1/~user18130814200115/\">tilde.club</a>.\n\n-----------------------------------------------------------------\n\nRecent posts:\n" >> index.html
...
generate_gophermap() {
    ...
    printf "\nhWorld Wide Web version of this phlog	URL:https://blog.wester.digital\nhRSS feed	URL:https://blog.wester.digital/feed.xml\n-----------------------------------------------------------------\n\nRecent posts:\n" >> gophermap
...
generate_rss_v2() {
    ...
    head -n 9999 posts/* | sed -Ee 's/>/\&amp;gt;/g' -e 's/</\&amp;lt;/g' -e 's/^==.amp.gt. (.*) .amp.lt.==$/<\/pre>]]><\/description><\/item><item><link>https:\/\/blog.wester.digital\/\1<\/link><title>/g' -e 's/([A-z].., [0-9]* [A-z].. [0-9]...)/<\/title><pubDate>\1<\/pubDate><description><![CDATA[<pre>/g'
